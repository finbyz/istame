...
def istame_overdue_email():
	...
	for row in data:
		...
		if doc.due_date_3 is not None and str(doc.due_date_3) <= now() and doc.escalation_second_sent == "0":
				...
				if doc.sp_details_email:
					recipients = split_emails(doc.sp_details_email.replace("\n", ""))
				level_4_supervisor_email = frappe.get_value("Service",filters={'name': doc.service_type},fieldname='level_4_supervisor_email')					
				recipients.extend(split_emails(level_4_supervisor_email.replace("\n", "")))
...
def istame_warning_email():
	...
	for row in data:
		doc = frappe.get_doc("Ista Middle East",row.name)
		recipients = []
		if doc.due_date_1 is not None and str(doc.due_date_1) <= now() and doc.warning_sent == "0":
